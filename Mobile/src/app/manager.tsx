...
const getMetrics = (t: (vi: string, en: string) => string) => [
  { label: t('Công suất', 'Occupancy'), value: '84%', detail: t('482 / 574 chỗ', '482 / 574 slots') },
...
const getAlerts = (t: (vi: string, en: string) => string) => [
  {
    ...
    detail: t('Dự báo 96% trong 18 phút, đã bật điều hướng lại', 'Predicted 96% in 18m, reroute active'),
  },
  {
    ...
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <ThemedText style={styles.eyebrow}>{t('Vai trò 01 - Quản lý cơ sở', 'Role 01 - Facility Manager')}</ThemedText>
          <ThemedText style={styles.title}>{t('Tóm tắt điều hành di động', 'Mobile command summary')}</ThemedText>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeaderRow}>
            <ThemedText style={styles.cardTitle}>{t('Cảnh báo ưu tiên', 'Priority alerts')}</ThemedText>
        ...
        <View style={styles.aiCard}>
          <ThemedText style={styles.aiEyebrow}>{t('AI - 2 giờ tới', 'AI - Next 2 hours')}</ThemedText>
