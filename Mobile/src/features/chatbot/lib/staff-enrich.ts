...
export type StaffChatIntent =
  "checkInFlow" | "reservationLookup" | "lotStatus" | "activeSessions" | "general";
...
function summarizeReservations(reservations: Reservation[]): string {
  const relevant = reservations.filter((r) => r.status === "PENDING" || r.status === "CLAIMED");
...
function summarizeActiveSessions(sessions: ParkingSession[]): string {
  ...
  return active
    .slice(0, 8)
    .map((s) => {
      const plate = typeof s.vehicleId === "object" ? (s.vehicleId.licensePlate ?? "?") : "?";
      const slot = typeof s.parkingSlotId === "object" ? (s.parkingSlotId.slotNumber ?? "?") : "?";
      return `- ${plate} tại slot ${slot}`;
    })
    .join("\n");
}

export async function listStaffReservationsForChat(limit = 20): Promise<Reservation[]> {
