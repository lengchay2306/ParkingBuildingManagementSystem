...
export type StaffPhoneValidation =
  { ok: true; phone: string } | { ok: false; messageKey: "empty" | "invalid" };
...
export function validateObjectIdInput(
  ...
  if (!id) {
    return {
      ok: false,
      message: t("Nhập mã phiên / đặt chỗ", "Enter a session or reservation ID"),
    };
  }
