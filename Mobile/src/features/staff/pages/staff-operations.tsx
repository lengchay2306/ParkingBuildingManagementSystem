...
export default function StaffOperationsScreen() {
    ...
    try {
      ...
      showToast(
        error instanceof Error
          ? error.message
          : t("Không xóa được đặt chỗ", "Cannot delete reservation"),
        "error",
      );
    } finally {
  ...
  function handleUnavailableAction(label: string) {
    showToast(
      t(`${label}: tính năng đang được phát triển.`, `${label}: feature is under development.`),
      "error",
    );
  }
    ...
    >
      <View style={styles.card}>
        <ThemedText style={styles.eyebrow}>
          {t("Ra cổng / Thanh toán", "Exit / Payment")}
        </ThemedText>
      ...
      <View style={styles.card}>
        <ThemedText style={styles.eyebrow}>
          {t("Xóa đặt chỗ PENDING", "Delete PENDING reservation")}
        </ThemedText>
        <StaffTextInput
