...
import { getParkingSlots, type ParkingFloor, type ParkingSlot } from "@/features/staff/api";
...
function resolveSpotTone(status: string): "available" | "occupied" | "unavailable" {
    ...
    try {
      ...
      setSlotsError(
        error instanceof Error
          ? error.message
          : t("Không tải được danh sách ô", "Could not load spots"),
      );
    } finally {
    ...
    >
      ...
      {isLoadingSlots ? (
        ...
        <View style={{ gap: 10 }}>
          {flatSlots.length === 0 ? (
            <ThemedText
              style={{ color: DesignColors.inkMuted, textAlign: "center", marginTop: 16 }}
            >
            ...
            flatSlots.map(({ slot, floorId, floorName }) => {
              ...
              const subtitle = activeSession ? `${floorName} · ${activeSession.plate}` : floorName;
